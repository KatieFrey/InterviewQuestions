...
output: [ ["practice", "3"], ["perfect", "2"],
          ...
          ["get", "1"], ["by", "1"], ["just", "1"] ]
Important: please convert the occurrence integers in the output list to strings (e.g. "3" instead of 3). We ask
this because in compiled languages such as C#, Java, C++, C etc., it’s not straightforward to create mixed-type
arrays (as it is, for instance, in scripted languages like JavaScript, Python, Ruby etc.). The expected output
will simply be an array of string arrays.
...
function wordCountEngine(input) {
  ...
  for (let i = 0; i < neutralized.length; i++) {
    let currentElem = neutralized[i];
    if (wordMap.has(currentElem)) {
      let value = Number(wordMap.get(currentElem));
      wordMap.set(currentElem, (value + 1).toString());
    } else {
      wordMap.set(currentElem, "1");
    }
  }
